...
function NotesList() {
    ...
    // 2. Update fetchNotes to handle the new API response and page number
    const fetchNotes = useCallback(async (searchParams, page) => {
        ...
        try {
            ...
            // 3. Set notes and pagination data from the new response structure
    ...
    useEffect(() => {
        const timer = setTimeout(() => {
            fetchNotes(filters, 1); // Always fetch page 1 when search filters change
        }, 500);
                            ...
                            <div key={note.id} className="note-card">
                                ...
                                {currentUserRole === 'moderator' && (
                                    <div className="note-actions">
                                        <button onClick={() => handleDelete(note.id)}><FaTrash /> Delete (Moderator)</button>
                                    </div>
                                )}
                            </div>
